...
新增 3 个菜系分类（粤 / 川 / 湘），并把「怎么挑食材」和「新手提示」写进每一道菜 ——
目标是**照着能买、照着能做**。

### 新增
- **菜系分类 6 → 9 个**：新增「粤菜 60 道」「川菜 60 道」「湘菜 60 道」，
  菜谱总数 **500 → 680 道**。
  - 粤菜覆盖烧腊、浸鸡、清蒸白灼海鲜、小炒、老火汤、粥粉面、广式点心
  - 川菜覆盖经典名菜、干煸爆炒、水煮烧菜、凉菜蘸水、面食小吃，
    也收了开水白菜 / 鸡豆花 / 甜烧白这类不辣的，体现川菜的另一面
  - 湘菜覆盖经典名菜、爆炒干锅、蒸菜、腌腊坛子菜、小吃
  - 三个新分类的 `steps` 按**小白标准**写：每步都带火候 + 时间 + 判断标准 + 常见坑，
    并把「七成热」「断生」「灯盏窝」「收汁挂勺」这类术语翻译成能上手判断的说法
- **食材选购 + 新手提示（680 道全覆盖，新增 `data/pick-{分类}.js` 9 个文件）**
  - `pick`：3~6 条，每道菜的主要食材与关键调料给出**能上手判断的特征**
    （看颜色 / 摸手感 / 闻气味 / 看包装标注），例：
    「五花肉：挑三层肥两层瘦、层数分明的；手指按下去能弹回来，表面发黏或渗水的不要」
  - `tips`：2~4 条，优先写「术语翻译」与「最容易翻车的地方」，例：
    「油温七成热 = 筷子插进油里周围冒小泡。油面开始冒青烟就是过头了，蛋会发苦」
  - **明确禁止**「挑选新鲜的」「注意火候」这类空话
- **配图 500 → 680 张**：新生成 180 张（粤 / 川 / 湘 各 60），覆盖率仍为 100%

### 界面
- **详情页**：食材清单里，主要食材下方直接显示该食材的选购要点（买菜时能对着看）；
  卡片下方新增「新手小技巧」区块（浅绿卡片，与主体区分）
- **做菜页**：备料清单后面新增「下锅前先看这几条」，把这道菜最关键的提示放在动火之前
- 选购数据按分类懒加载（`DATA.picksOf(id)`），与菜谱数据并行取，不阻塞首屏

### 工程
- 工具链分类清单统一扩到 9 个（`gen-manifest` / `validate-data` / `list-recipes` /
  `missing-images` / `build_img_map` / `compress_images`）
