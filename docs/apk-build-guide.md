...
bash tools/verify_apk.py dist/chishenme-v3.1.0.apk \
  "C:/Users/q2764/.workbuddy/binaries/android-build/work-chishenme/build/dex/classes.dex" \
  src/index.html
```

产物：`dist/chishenme-v3.1.0.apk`（约 3.2MB）+ 中文名副本。
...
"$BT/aapt2.exe" link -o "$BUILD/base.apk" -I "$PLATFORM" \
  ...
  --version-code 31 --version-name "3.1.0" "$BUILD/res.zip"
...
"$BT/apksigner.bat" sign --ks "$BASEW/keys/chishenme.keystore" \
  ...
  --v4-signing-enabled false --out chishenme-v3.1.0.apk "$BUILD/aligned.apk"
```

## 四、验证

```bash
"$BT/apksigner.bat" verify --verbose chishenme-v3.1.0.apk   # 期望 v2/v3 = true
"$BT/zipalign.exe" -c 4 chishenme-v3.1.0.apk                # 期望无输出
"$BT/aapt2.exe" dump badging chishenme-v3.1.0.apk | grep uses-permission   # 期望无输出（零权限）
python tools/verify_apk.py chishenme-v3.1.0.apk <dex> <src/index.html>
```
